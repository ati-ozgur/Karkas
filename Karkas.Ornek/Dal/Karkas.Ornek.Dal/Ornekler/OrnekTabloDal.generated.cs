...
public partial class OrnekTabloDal : BaseDal<OrnekTablo>
{
	...
	protected override void ProcessRow(IDataReader dr, OrnekTablo row)
	{
		...
		if (!dr.IsDBNull(2))
		{
			row.KolonBinary = (Byte[])dr.GetValue(2);
		}
		if (!dr.IsDBNull(3))
		{
	...
	protected override void InsertCommandParametersAdd(DbCommand cmd, OrnekTablo row)
	{
		...
		builder.parameterEkle("@KolonBinary",SqlDbType.Binary, row.KolonBinary);
		...
		builder.parameterEkle("@KolonImage",SqlDbType.Image, row.KolonImage);
		...
		builder.parameterEkle("@KolonNText",SqlDbType.NText, row.KolonNtext);
		...
		builder.parameterEkle("@KolonText",SqlDbType.Text, row.KolonText);
		...
		builder.parameterEkle("@KolonVarBinary",SqlDbType.VarBinary, row.KolonVarBinary);
