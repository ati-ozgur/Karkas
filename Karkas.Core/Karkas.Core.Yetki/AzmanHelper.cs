...
namespace Karkas.Core.Yetki
{
    public class AzmanHelper
    {
        ...
        private IAzApplication Application
        {
            get
            {
                if (this.azApplication == null)
                {
                    ...
                    {
                        throw new Exception("IAzAuthorizationStore.Initialize cagrısında sorun çıktı.", x);
                    }
                    ...
                    catch (System.Runtime.InteropServices.COMException x)
                    {
                        Release(azManStore);
                        azManStore = null;
                        throw new Exception("IAzAuthorizationStore.OpenApplication uygulamanın açılmasında sırasında COM hatası", x);
                    }
                }
        ...
        private IAzClientContext GetClientContext(string username)
        {
            ...
            {
                throw new ArgumentException("username null olmamalıdır");
            }
        ...
        private void initClientContext(string user, string domain)
        {
            ...
            {
                throw new ArgumentException("Kullanıcı adı bu domain/pc'de tanımlı değil", ex);
            }
        }
